...
<body>
<header class="quran-nav"><div class="quran-wrap"><a href="index.php?page=quran<?=e($quranContext)?>">← Daftar Surat</a><b>Baca Al-Qur’an</b><a href="index.php?page=<?=!empty($quranFromDashboard)?'dashboard':'home'?>"><?=!empty($quranFromDashboard)?'Dashboard':'Beranda'?></a></div></header>
<main class="quran-wrap reading-main">
<?php if($publicSurahError):?><section class="read-error"><b>Ayat belum dapat ditampilkan</b><p><?=e($publicSurahError)?> Periksa koneksi internet lalu coba kembali.</p><a href="index.php?page=quran-read&surah=<?=(int)($publicSurah['surah_number']??0)?><?=e($quranContext)?>">Coba Lagi</a></section>
<?php else:?>
<article class="reader" data-surah="<?=(int)$publicSurahDetail['nomor']?>" data-name="<?=e($publicSurahDetail['namaLatin'])?>">
    ...
    <nav class="reader-pagination" aria-label="Navigasi surat"><?php if($previousSurah):?><a class="previous" href="index.php?page=quran-read&surah=<?=(int)$previousSurah['surah_number']?><?=e($quranContext)?>"><i>←</i><span><small>Surat Sebelumnya</small><b><?=e($previousSurah['name'])?></b></span></a><?php else:?><span class="reader-nav-placeholder"></span><?php endif?><?php if($nextSurah):?><a class="next" href="index.php?page=quran-read&surah=<?=(int)$nextSurah['surah_number']?><?=e($quranContext)?>"><span><small>Surat Berikutnya</small><b><?=e($nextSurah['name'])?></b></span><i>→</i></a><?php endif?></nav>
</article><?php endif?>
</main><script src="assets/quran.js?v=20260811c"></script></body></html>
