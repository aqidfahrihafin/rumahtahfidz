...
<section class="history-page-head">
    ...
    <?php if ($historyType === 'assessment' && $role !== 'wali'): ?><button class="btn primary" type="button" onclick="openForm()">＋ Tambah Penilaian</button><?php endif; ?>
</section>
...
document.querySelectorAll('input[name="action"][value="send_report_email"]').forEach(function (actionInput) {
    var returnInput = document.createElement('input'); returnInput.type = 'hidden'; returnInput.name = 'return'; returnInput.value = 'history';
    var studentInput = document.createElement('input'); studentInput.type = 'hidden'; studentInput.name = 'student_id'; studentInput.value = <?=json_encode((string)$historyStudentId)?>;
    actionInput.form.appendChild(returnInput); actionInput.form.appendChild(studentInput);
});
</script>
<?php endif; ?>
